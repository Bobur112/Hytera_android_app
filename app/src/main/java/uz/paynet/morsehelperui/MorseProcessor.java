...
import uz.paynet.morsehelperui.WavFile;
...
public class MorseProcessor {
    ...
    private double SHOVQIN = 0.02;

    public double getSHOVQIN() {
        return SHOVQIN;
    }

    public void setSHOVQIN(double SHOVQIN) {
        this.SHOVQIN = SHOVQIN;
    }

    public MorseProcessor(File file) throws WavFileException, IOException {
    ...
    public MorseSignal readNextSignal() throws WavFileException, IOException {
        ...
        if (sample > SHOVQIN) { // if the sample value is greater than 0.01 it is a valid signal
        ...
        while (this.wavFile.getFramesRemaining() > 0) {
            ...
            if ((signal.isSilence && tmp_sample > SHOVQIN) // if we got a silence and now a signal
                    || (signal.isSignal && tmp_sample <= SHOVQIN)) // if we got a signal and now a silence
            {
    ...
    private List<String> finalize(List<MorseSignal> signals) {
        ...
        if (signals_length.size() != 0 && silences_length.size() != 0) { /** 19.01.2024 qo'yildi*/
        ...
        System.out.println("this.signal_medium_value: " + this.signal_medium_value);
        System.out.println("this.silence_medium_value: " + this.silence_medium_value);
        ...
        while (second_iterator.hasNext()) {
            ...
            if (signal.isSignal) {
                System.out.print("s" + signal.length_ms());
